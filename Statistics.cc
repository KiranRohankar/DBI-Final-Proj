...
void Statistics::AddRel(char *relName, int numTuples)
{
	...
	if(it==this->m_rel_stat.end()) { //not found, add it
...
void Statistics::AddAtt(char *relName, char *attName, int numDistincts)
{
	...
	if(relIt == this->m_rel_stat.end()) { // not found
	...
	if(attIt==rel->atts.end()) { //not found, add
		...
		if(it==this->m_att_to_rel.end()) { //not found add
			vector<string> *relNames = new vector<string>;
			relNames->push_back(string(relName));
			this->m_att_to_rel.insert(make_pair(string(attName), relNames));
		} else
...
void Statistics::CopyRel(char *oldName, char *newName)
{
	...
	if(oIt == this->m_rel_stat.end()) { // old relation not found
...
//		tr1::unordered_map <string, unsigned long int>::iterator attIt;
//		tr1::unordered_map <string, vector<string>*>::iterator a2rIt;
...
void Statistics::Write(char *fromWhere)
{
	...
	for(tr1::unordered_map <string, Rel_Stat *>::iterator rIt = this->m_rel_stat.begin();
		...
		if(!r->copied) {
			...
			for(tr1::unordered_map <string, unsigned long int>::iterator aIt = r->atts.begin();
						aIt!=r->atts.end(); aIt++) {
				//write out its att
...
//	for(tr1::unordered_map <string, Rel_Stat *>::iterator it=this->m_rel_stat.begin(); it!=this->m_rel_stat.end();it++) {
//		cout <<"Relation "<<it->first<<" partition is: " <<it->second->partition<<endl;
//	}
//	tr1::unordered_map <int, Partition*>::iterator pit = this->m_partition.find(m_partitionNum-1);
...
/*	set <int> join_partitions;
	...
	for(set<int>::iterator sIt = join_partitions.begin(); sIt!=join_partitions.end(); sIt++) {
		// partition
		tr1::unordered_map <int, Partition*>::iterator pIt = this->m_partition.find(*sIt);
...
*/
	...
	if(!parseTree) {  // parseTree is null
...
//			cout <<"get another orlist"<<endl;
			...
			if(left->code == NAME && right->code == NAME) {  //-------- join condition ------
				//att1 = att2
...
//				tr1::unordered_map <string, Rel_Stat *>::iterator rIt = this->m_rel_stat.find(string(left->value));
//				if()
